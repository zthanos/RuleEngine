...
{
    public static object? OperandTypeResolver(JSchema schema, string leftOperand, string rightOperandRaw)
    {
        ...
        if (propertySchema != null)
        {
            ...
            switch (propertySchema.Type)
            {
                case JSchemaType.Integer:
                    // Parse as integer
                    return int.TryParse(rightOperandRaw, out int intValue) ? intValue : default;

                case JSchemaType.Number:
                    // Parse as double or float, depending on required precision
                    return double.TryParse(rightOperandRaw, out double doubleValue) ? doubleValue : default;

                case JSchemaType.Boolean:
                    // Parse as boolean
                    return bool.TryParse(rightOperandRaw, out bool boolValue) && boolValue;
